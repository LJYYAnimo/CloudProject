layui.use(['table', 'upload', 'form'], function () {
    ...
    table.render({
        ...
        , cols: [[
            ...
            , {field: 'worksType', title: '类型', align: 'center',edit: 'text'}
    ...
    table.on('tool(demo)', function (obj) {
        ...
        if (obj.event === 'del') {
            layer.msg('确定删除？', {
                ...
                , btn: ['删除', '取消']
                , yes: function (index) {
                    axios.post('/worksType/delete', Qs.stringify(data)).then(function (response) {
                        if (response.data.code == 0) {
                            layer.close(index);
                            layer.msg(response.data.message, {icon: 6});
                            reloads();
            ...
            form.val('worksTypeForm',{
                "id":data.id,
                "worksType":data.worksType
            });
            url ='/worksType/update';
            openAddUpdate('作品类型更新');
